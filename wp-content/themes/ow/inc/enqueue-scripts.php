...
if (!function_exists('crunch_enqueue_scripts')) :
    function crunch_enqueue_scripts()
    {
        ...
        if (file_exists(plugin_dir_path(__FILE__) . '../' . $block_investments_uri)) {
            $block_investments_styles_ver = date("ymd-Gis", filemtime(plugin_dir_path(__FILE__) . '../' . $block_investments_styles_uri));

            wp_enqueue_style('acf-block-investments', get_template_directory_uri() . '/' . $block_investments_styles_uri, false, $block_investments_styles_ver);
        }
